...
(function() {
  var app_version, check, clog, cltags, coffeec, compile, create, http, isPortTaken, jadec, jjson, lookup, minify, net, npm, sjson, startLiveReload, tags, tinylr, watcher;
  ...
  if (tags.command === "watch") {
    ...
    watcher.watch(function(file) {
      if (file.substr(-5) === ".styl") {
        return compile(watcher.stylFile, tags.name, tags.outpath, function(remf) {
          if (tags.livereload) {
            http.get("http://localhost:35729/changed?files=" + remf);
            return console.log("\x1B[0;32m" + remf.split("/").pop() + " modified & reloaded\x1B[0;0m");
          }
        });
      } else if (file.substr(-3) === ".js" && file.substr(-11) !== "minified.js") {
        return minify(watcher.jsDir);
      } else if (file.substr(-7) === ".coffee") {
        return coffeec(file);
      } else if (file.substr(-5) === ".jade") {
        return jadec(file);
    ...
    if (!tags.ignore) {
      ...
      (lookup = function() {
        var p;
        if (check.length === 0) {
          return create(tags.query, tags.ignore, tags.debug);
        } else {
          p = check.shift();
